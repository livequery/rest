...
function catchError2(selector) {
  return operate(function(source, subscriber) {
    ...
    innerSub = source.subscribe(createOperatorSubscriber(subscriber, undefined, undefined, function(err) {
      handledResult = innerFrom(selector(err, catchError2(selector)(source)));
      if (innerSub) {
...
function first2(predicate, defaultValue) {
  ...
  return function(source) {
    return source.pipe(predicate ? filter2(function(v, i) {
      return predicate(v, i, source);
    }) : identity, take2(1), hasDefaultValue ? defaultIfEmpty2(defaultValue) : throwIfEmpty2(function() {
      return new EmptyError;
    }));
  };
}
...
class Socket extends BehaviorSubject {
  ...
  #init() {
    ...
    if (this.#running)
      return;
    this.#running = of(1).pipe(takeUntil2(this.pipe(ignoreElements2())), mergeMap(async () => new WebSocket(this.endpoint)), switchMap2((ws) => merge(fromEvent(ws, "closed").pipe(map((e) => {
      throw e;
    })), fromEvent(ws, "close").pipe(map((e) => {
      throw e;
    })), fromEvent(ws, "error").pipe(map((e) => {
      throw e;
    })), fromEvent(ws, "open").pipe(switchMap2(() => interval(60000)), tap2(() => ws.send(JSON.stringify({ event: "ping" })))), fromEvent(ws, "open").pipe(tap2(() => {
      this.next({
        ...this.value,
        connected: true,
    ...
    }))).pipe(finalize2(() => ws.close()))), catchError2((e) => {
      this.$connected.next(false);
      throw e;
    }), retry2({ delay: 1000 })).subscribe();
  }
  stop() {
    this.complete();
  }
  ...
  listen(ref) {
    ...
    if (!topic)
      return EMPTY;
    topic.listen_count++;
    return topic.stream.pipe(finalize2(() => {
      topic.listen_count--;
      setTimeout(() => {
...
class RestTransporter {
  ...
  constructor(config3) {
    this.config = config3;
    if (config3.ws) {
      this.socket = new Socket(config3.ws);
    }
  }
  async#call(req) {
