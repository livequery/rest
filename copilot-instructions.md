...
- Treat `@livequery/rest` as a reusable transport adapter for `@livequery/core`.
- Edit `src/`, never `dist/`.
...
- Generate consumer examples around a shared `RestTransporter` installed into a shared `LivequeryCore` instance.
...
- Do not move collection-state responsibilities into this package; that logic belongs to `@livequery/core`.
